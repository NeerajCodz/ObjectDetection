...
def process(image, model_name):
    """Process an image and return detected image, objects, confidences, unique objects, and unique confidences."""
    try:
        if model_name not in VALID_MODELS:
            raise ValueError(f"Invalid model: {model_name}. Choose from: {VALID_MODELS}")
        ...
        return image, object_names, confidence_scores, unique_objects, unique_confidences
    except Exception as e:
        logger.error(f"Error in process: {str(e)}\n{traceback.format_exc()}")
        raise

# Get image properties (for Gradio UI)
...
# Generate API documentation (for Gradio UI)
...
async def detect_objects_endpoint(
    ...
    try:
        ...
        detected_image, detected_objects, detected_confidences, unique_objects, unique_confidences = process(image, model_name)
    ...
    with gr.Blocks(theme=gr.themes.Default(primary_hue="blue", secondary_hue="gray")) as demo:
        ...
        with gr.Tabs():
            with gr.Tab("📷 Image Upload"):
                ...
                def process_for_gradio(image, model_name):
                    try:
                        detected_image, objects, scores, unique_objects, unique_scores = process(image, model_name)
                        ...
                        return detected_image, objects_df, properties_text, unique_objects_df, ""
                    except Exception as e:
                ...
                submit_btn.click(
                    ...
                    outputs=[output_image, objects_output, properties_output, unique_objects_output, error_output]
                )
                ...
                clear_btn.click(
                    fn=clear_inputs,
                    inputs=None,
                    outputs=[image_input, output_image, objects_output, unique_objects_output, error_output]
                )
            ...
            with gr.Tab("🔌 Use via API"):
                ...
                def process_for_api(image, model_name):
                    try:
                        detected_image, objects, scores, unique_objects, unique_scores = process(image, model_name)
                        buffered = BytesIO()
                        detected_image.save(buffered, format="PNG")
